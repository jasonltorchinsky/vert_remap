!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! Implementation of the vertical remapping algorithm of Colella, Sekora 2008.
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

submodule (vertremap_mod) vertremap_cs08
...
contains
  ...
  module subroutine remap_cs08_sbr(qdp, ncell, dp1, dp2, remap_alg, verbosity)
    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ! Set up variables for modified PPM.
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    !  the interior, and global bounded of the parabolic pieces.
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!!$    end if
  ...
  function correct_edgeval(ncell, dp1, avgdens, edgeval, C, cell) &
    ...
    ! edge value for.
    real(real64)               :: D2a, D2aL, D2aR, D2aLim ! Second derivative
    ! approximations.
    real(real64)               :: D2aSpc, D2aLSpc, D2aRSpc, D2aLimSpc
    real(real64)               :: corr_edgeval ! Corrected edge value
    real(real64)               :: temp_dp ! Holds working real variables

    temp_dp = 0.0_real64

    ! Calculate second derivative approximations.
    !! Centered
    D2aSpc = (1.0_real64 / 6.0_real64) &
         & * (dp1(cell+1) + dp1(cell))
    D2a = 8.0_real64 * D2aSpc**2 * &
         & ( avgdens(cell) / (dp1(cell) * (dp1(cell+1) + dp1(cell))) &
         &   - edgeval / (dp1(cell) * dp1(cell+1)) &
         &   + avgdens(cell+1) / (dp1(cell+1) * (dp1(cell+1) + dp1(cell))) )

    !! Left
    D2aLSpc = (1.0_real64 / 6.0_real64) &
         & * (dp1(cell+1) + 3.0_real64 * dp1(cell) + dp1(cell-1))
    D2aL = 8.0_real64 * D2aLSpc**2 * &
         & ( avgdens(cell-1) &
    ...
    !! Right
    D2aRSpc = (1.0_real64 / 6.0_real64) &
         & * (dp1(cell+2) + 3.0_real64 * dp1(cell+1) + dp1(cell))
    D2aR = 8.0_real64 * D2aRSpc**2 * &
         & ( avgdens(cell) &
         &     / ((dp1(cell+1) + dp1(cell)) &
    ...
    if ((D2a * D2aL .gt. 0) .and. (D2aL * D2aR .gt. 0)) then
       D2aLim = sign( min(C * abs(D2aL), C * abs(D2aR), abs(D2a)), D2a )
       ! Set the spacing parameter
       temp_dp = min(C * abs(D2aL), C * abs(D2aR), abs(D2a))
       if (temp_dp .eq. C * abs(D2aL)) then
          D2aLimSpc = D2aLSpc
       else if (temp_dp .eq. C * abs(D2aR)) then
          D2aLimSpc = D2aRSpc
       else
          D2aLimSpc = D2aSpc
       end if
       ...
       D2aLim = 0.0_real64
       D2aLimSpc = 1.0_real64
    end if

    corr_edgeval = (avgdens(cell) + avgdens(cell+1))/2.0_real64 &
         & - (1.0_real64/6.0_real64) * D2aLim * D2aLimSpc**2
  ...
  subroutine correct_parabvals(ncell, dp1, avgdens, parabvals, C, cell)
    ...
    integer(int32), intent(in) :: cell ! Current cell we are correcting the
    ! edge value for.
    real(real64) :: D2a, D2aL, D2aC, D2aR, D2aLim
    integer(int32) :: s
    real(real64) :: alphajp, alphajm
    real(real64) :: dIext, da
    ...
    if (((parabvals(2) - avgdens(cell)) * (avgdens(cell) - parabvals(1)) .le. 0.0_real64) &
       ...
       D2a = 4.0_real64 / (dp1(cell)**2) * (parabvals(1) - 2.0_real64 * avgdens(cell) + parabvals(2))
       D2aL = 8.0_real64 * &
       ...
       if ((D2a * D2aL .gt. 0.0_real64) &
            & .and. (D2aL * D2aC .gt. 0.0_real64) &
            & .and. (D2aC * D2aR .gt. 0.0_real64)) then
          D2aLim = sign( min(abs(D2a), C * abs(D2aL), C * abs(D2aC), C * abs(D2aR)), D2a  )
       else
          D2aLim = 0.0_real64
       end if
       ...
       if (D2a .ne. 0) then
          parabvals(1) = avgdens(cell) + (parabvals(1) - avgdens(cell)) * D2aLim / D2a
          parabvals(2) = avgdens(cell) + (parabvals(2) - avgdens(cell)) * D2aLim / D2a
       else
       ...
       alphajm = parabvals(1) - avgdens(cell) ! alpha_{j,-}
       alphajp = parabvals(2) - avgdens(cell) ! alpha_{j,+}

       if (abs(alphajp) .gt. 2.0_real64 * abs(alphajm)) then
          dIext = -1.0_real64 * alphajp**2 / (4.0_real64 * (alphajp + alphajm)) ! delta I_ext
          da = avgdens(cell+1) - avgdens(cell) ! delta a
          if (s * dIext .ge. s * da) then
             parabvals(2) = avgdens(cell) &
                  & - 2.0_real64 * (da + s * sqrt(da**2 - da * alphajm))

          end if

       else if (abs(alphajm) .gt. 2.0_real64 * abs(alphajp)) then
          dIext = -1.0_real64 * alphajm**2 / (4.0_real64 * (alphajp + alphajm)) ! delta I_ext
          da = avgdens(cell-1) - avgdens(cell) ! delta a
          if (s * dIext .ge. s * da) then
             parabvals(1) = avgdens(cell) &
                  & - 2.0_real64 * (da + s * sqrt(da**2 - da * alphajp))
