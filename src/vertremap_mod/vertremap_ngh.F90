...
submodule (vertremap_mod) vertremap_ngh
...
contains
  ...
  module subroutine ngh_remap_sbr(qdp, ncell, dp1, dp2, remap_alg, verbosity)
    ...
    ! Once we have the initial interface value estimates, adjust the ghost-cell
    ! data to be the same as the boundary cells
!!$    do ii = -1, 0
!!$       avgdens(ii) = avgdens(1)
!!$    end do
!!$    do ii = ncell+1, ncell+1
!!$       avgdens(ii) = avgdens(ncell)
!!$    end do
    ...
    if (remap_alg .eq. 22) then ! Flatten boundary parabolas
       parabvals(1:2, 1) = avgdens(1)
       parabvals(3, 1) = 6.0_real64 * avgdens(1) &
            & - 3.0_real64 * (parabvals(1, 1) + parabvals(2, 1))

       parabvals(1:2, ncell) = avgdens(ncell)
       parabvals(3, ncell) = 6.0_real64 * avgdens(ncell) &
            & - 3.0_real64 * (parabvals(1, ncell) + parabvals(2, ncell))
    end if
