...
module vertremap_mod
  ...
  ! Declare variable types based on the standard iso_fortran_env
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       enddo
    enddo ! q loop
    if (abrtf) error stop 'Bad levels in remap1, usually CFL violation'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       enddo
    enddo ! q loop
    if (abrtf) error stop 'Bad levels in remap1 nofilter, usually CFL violation'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do j = 1 , nx
       do i = 1 , nx
          ...
          do q = 1 , qsize
             ...
             coefs(:,:) = compute_ppm( ao , ppmdx, nlev, remap_alg )
  ...
  function compute_ppm( a , dx, nlev, remap_alg )    result(coefs)
    implicit none
    integer(kind=int_kind), intent(in) :: nlev
