...
contains
  ...
  module subroutine new_remap_sbr(qdp, ncell, dp1, dp2, remap_alg)
    ...
    ! calculations.
    integer(int32)              :: temps_int(1) ! Temporary integer values used
    ! in calculations.
       ...
       do kk = 1, interpord
          ...
          do ll = 1, interpord
             ...
             if (lidx .ne. kidx) then
                if (jj .ne. lidx) then ! Beware the zero in the denomiator!
                   temps_dp(2) = temps_dp(2) + 1.0_real64 / (grid1(jj) - grid1(lidx))
                end if
                temps_dp(3) = temps_dp(3) &
                     & * (grid1(jj) - grid1(lidx))/(grid1(kidx) - grid1(lidx))
             end if
          end do
          temps_dp(1) = temps_dp(1) + totmass(kidx)*temps_dp(2)*temps_dp(3)
       end do
