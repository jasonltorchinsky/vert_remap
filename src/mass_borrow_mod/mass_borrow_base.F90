...
contains
  ...
  subroutine borrow_mass(ncell, Qdp, dp2, ubnds, lbnds)
    ...
    real(real64), intent(in) :: ubnds(ncell), lbnds(ncell) ! Upper and lower bound for each
    ...
    do ii = 1, (ncell-1)/2
       ...
       ubnd = ubnds(ncell-ii+1)
       lbnd = lbnds(ncell-ii+1)
       if (dens - ubnd .gt. tol) then
